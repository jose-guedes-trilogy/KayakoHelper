    ...
    const [{ result } = { result: null }] = await chrome.scripting.executeScript({
        ...
        func: async (injectedTimeoutMs: number) => {
            ...
            await (async () => {
                ...
                const idpRe = /[a-z0-9-]+:[0-9a-fA-F-]{8,}/i;
...
// credProbeClient.ts — content-side helper to ask background to run CredProbe in a pinned tab
export async function runCredProbeInPinnedTab(): Promise<{
    info: any; creds: any; idTok: string | null;
}> {
    const res = await chrome.runtime.sendMessage({
        type: "credprobe.run",
        originUrl: "https://dash.alpha.school/",
        path: "/user/list",     // ensure we hit a route that hydrates Amplify
        timeoutMs: 20000
    });
    if (!res || !res.ok) {
        throw new Error(res?.error || "CredProbe failed.");
    }
    return { info: res.info, creds: res.creds, idTok: res.idTok ?? null };
}
